...
void UI_PushMenu(menuFrameWork_t *menu)
{
    ...
    // CPP: Keydest_t
    Key_SetDest((KeyInputDestination)((Key_GetDest() & ~KEY_CONSOLE) | KEY_MENU));
...
void UI_ForceMenuOff(void)
{
    ...
    // CPP: Keydest
    Key_SetDest((KeyInputDestination)(Key_GetDest() & ~KEY_MENU));
    uis.menuDepth = 0;
    uis.activeMenu = NULL;
...
=================
*/
void UI_OpenMenu(UserInterfaceMenuType type)
{
...
void *UI_FormatColumns(int extrasize, ...)
{
    ...
    // CPP:
    buffer = (char*)UI_Malloc(extrasize + total + 1);
    p = buffer + extrasize;
    for (j = 0; j < i; j++) {
