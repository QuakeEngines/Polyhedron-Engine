...
static void load_material_image(image_t** image, const char* filename, pbr_material_t* mat, imagetype_t type, imageflags_t flags) {
	*image = IMG_Find(filename, type, (imageflags_t)(flags | IF_EXACT | (mat->image_flags & IF_SRC_MASK)));
	if (*image == R_NOTEXTURE) {
		...
		if (strncmp(filename, "overrides/", 10) == 0)
			*image = IMG_Find(filename, type, (imageflags_t)(flags | IF_EXACT));
	}
}
...
pbr_material_t* MAT_Find(const char* name, imagetype_t type, imageflags_t flags) {
	...
	if (matdef) 	{
		...
		if (mat->filename_base[0]) {
			load_material_image(&mat->image_base, mat->filename_base, mat, type, (imageflags_t)(flags | IF_SRGB));
			if (mat->image_base == R_NOTEXTURE) {
				...
				mat->image_base = IMG_Find(name, type, (imageflags_t)(flags | IF_SRGB));
				if (mat->image_base == R_NOTEXTURE) {
					mat->image_base = NULL;
				}
			} else {
		...
		if (mat->filename_emissive[0]) {
			load_material_image(&mat->image_emissive, mat->filename_emissive, mat, type, (imageflags_t)(flags | IF_SRGB));
		...
		if (mat->filename_mask[0]) {
			mat->image_mask = IMG_Find(mat->filename_mask, type, (imageflags_t)(flags | IF_EXACT | (mat->image_flags & IF_SRC_MASK)));
		...
		mat->image_emissive = IMG_Find(file_name, type, (imageflags_t)(flags | IF_SRGB));
		if (mat->image_emissive == R_NOTEXTURE)
			mat->image_emissive = NULL;
		else
