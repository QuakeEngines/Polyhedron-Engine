...
LOAD_EXT(BrushSides) {
    ...
    bsp->numbrushsides = count;
    bsp->brushsides = (mbrushside_t*)ALLOC(sizeof(*out) * count);

    in = (dbrushside_qbsp_t*)base;
    out = bsp->brushsides;
    for (i = 0; i < count; i++, in++, out++) {
...
LOAD_EXT(LeafBrushes) {
    ...
    bsp->numleafbrushes = count;
    bsp->leafbrushes = (mbrush_t**)ALLOC(sizeof(*out) * count);
...
LOAD_EXT(Edges) {
    ...
    bsp->numedges = count;
    bsp->edges = (medge_t*)ALLOC(sizeof(*out) * count);

    in = (dedge_qbsp_t*)base;
    out = bsp->edges;
    for (i = 0; i < count; i++, out++, in++) {
        for (j = 0; j < 2; j++) {
...
LOAD_EXT(Faces) {
    ...
    bsp->numfaces = count;
    bsp->faces = (mface_t*)ALLOC(sizeof(*out) * count);

    in = (dface_qbsp_t*)base;
    out = bsp->faces;
    for (i = 0; i < count; i++, in++, out++) {
        firstedge = LittleLong(in->firstedge);
...
LOAD_EXT(LeafFaces) {
    ...
    bsp->numleaffaces = count;
    bsp->leaffaces = (mface_t**)ALLOC(sizeof(*out) * count);
...
LOAD_EXT(Leafs) {
    ...
    bsp->numleafs = count;
    bsp->leafs = (mleaf_t*)ALLOC(sizeof(*out) * count);

    in = (dleaf_qbsp_t*)base;
    out = bsp->leafs;
    for (i = 0; i < count; i++, in++, out++) {
        out->plane = NULL;
        out->contents = LittleLong(in->contents);
...
LOAD_EXT(Nodes) {
    ...
    bsp->numnodes = count;
    bsp->nodes = (mnode_t*)ALLOC(sizeof(*out) * count);

    in = (dnode_qbsp_t*)base;
    out = bsp->nodes;
    for (i = 0; i < count; i++, out++, in++) {
        planenum = LittleLong(in->planenum);
        if (planenum >= bsp->numplanes) {
                               ...
                               mnode_t *headNode, const vec3_t &origin, vec3_t *angles)
{
    ...
    if (angles) {
        AnglesToAxis(*angles, axis); // MATHLIB: !!!! Does this work?
