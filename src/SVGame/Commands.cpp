...
==================
*/
void Cmd_Give_f(PlayerEntity *playerEntity)
{
    const char        *name;
    gitem_t     *it;
    int         index;
    ...
    if (give_all || Q_stricmp(gi.argv(1), "health") == 0) {
        if (gi.argc() == 3)
            playerEntity->SetHealth(atoi(gi.argv(2)));
        else
    ...
    if (give_all || Q_stricmp(name, "weapons") == 0) {
        for (i = 0 ; i < game.numberOfItems ; i++) {
            ...
            if (!(it->flags & ItemFlags::IsWeapon))
                continue;
            playerEntity->GetClient()->persistent.inventory[i] += 1;
        }
        if (!give_all)
            return;
    }
    ...
    if (give_all || Q_stricmp(name, "ammo") == 0) {
        for (i = 0 ; i < game.numberOfItems ; i++) {
            ...
            if (!(it->flags & ItemFlags::IsAmmo))
                continue;
            SVG_AddAmmo(playerEntity, it, 1000);
        }
        if (!give_all)
            return;
    }
    ...
    if (it->flags & ItemFlags::IsAmmo) {
        if (gi.argc() == 3)
            playerEntity->GetClient()->persistent.inventory[index] = atoi(gi.argv(2));
        else
            playerEntity->GetClient()->persistent.inventory[index] += it->quantity;
    } else {
        it_ent = SVG_Spawn();
        it_ent->className = it->className;
        SVG_SpawnItem(playerEntity, it);
...
==================
*/
void Cmd_God_f(SynchedEntityBase *serverEntity)
{
    if (deathmatch->value && !sv_cheats->value) {
    ...
    serverEntity->SetFlags(serverEntity->GetFlags() ^ EntityFlags::GodMode);
    if (!(serverEntity->GetFlags() & EntityFlags::GodMode))
        gi.CPrintf(serverEntity->GetEntityServerHandle(), PRINT_HIGH, "godmode OFF\n");
    else
        gi.CPrintf(serverEntity->GetEntityServerHandle(), PRINT_HIGH, "godmode ON\n");
}
...
==================
*/
void Cmd_Notarget_f(SynchedEntityBase *serverEntity)
{
    if (deathmatch->value && !sv_cheats->value) {
        gi.CPrintf(serverEntity->GetEntityServerHandle(), PRINT_HIGH, "You must run the server with '+set cheats 1' to enable this command.\n");
        return;
...
==================
*/
void Cmd_Noclip_f(SynchedEntityBase *serverEntity)
{
    if (deathmatch->value && !sv_cheats->value) {
        gi.CPrintf(serverEntity->GetEntityServerHandle(), PRINT_HIGH, "You must run the server with '+set cheats 1' to enable this command.\n");
        return;
...
void Cmd_Use_f(PlayerEntity *ent)
{
    ...
    if (!it) {
        gi.CPrintf(ent->GetEntityServerHandle(), PRINT_HIGH, "unknown item: %s\n", s);
        return;
    }
    if (!it->Use) {
        gi.CPrintf(ent->GetEntityServerHandle(), PRINT_HIGH, "Item is not usable.\n");
        return;
    }
    index = ITEM_INDEX(it);
    if (!ent->GetClient()->persistent.inventory[index]) {
        gi.CPrintf(ent->GetEntityServerHandle(), PRINT_HIGH, "Out of item: %s\n", s);
        return;
    }
...
void Cmd_Drop_f(PlayerEntity*ent)
{
    ...
    if (!it->Drop) {
        gi.CPrintf(ent->GetEntityServerHandle(), PRINT_HIGH, "Item is not dropable.\n");
        return;
    }
    index = ITEM_INDEX(it);
    if (!ent->GetClient()->persistent.inventory[index]) {
...
void Cmd_InvUse_f(PlayerEntity *ent)
{
    ...
    if (ent->GetClient()->persistent.selectedItem == -1) {
        gi.CPrintf(ent->GetEntityServerHandle(), PRINT_HIGH, "No item to use.\n");
        return;
    }
...
void Cmd_InvDrop_f(PlayerEntity *ent)
{
    ...
    if (ent->GetClient()->persistent.selectedItem == -1) {
        gi.CPrintf(ent->GetEntityServerHandle(), PRINT_HIGH, "No item to drop.\n");
        return;
    }
...
=================
*/
void Cmd_Players_f(PlayerEntity *playerEntity)
{
    int     i;
    int     count;
    char    small[64];
    ...
    gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_HIGH, "%s\n%i players\n", large, count);
}
...
=================
*/
void Cmd_Wave_f(PlayerEntity *playerEntity)
{
    // Fetch the action.
    int32_t i = atoi(gi.argv(1));

    // can't wave when ducked
    if (playerEntity->GetClient()->playerState.pmove.flags & PMF_DUCKED)
        return;

    if (playerEntity->GetClient()->animation.priorityAnimation > PlayerAnimation::Wave)
        return;

    playerEntity->GetClient()->animation.priorityAnimation = PlayerAnimation::Wave;

    switch (i) {
    case 0:
        gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_HIGH, "flipoff\n");
        playerEntity->GetState().frame = FRAME_flip01 - 1;
        playerEntity->GetClient()->animation.endFrame = FRAME_flip12;
        break;
    case 1:
        gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_HIGH, "salute\n");
        playerEntity->GetState().frame = FRAME_salute01 - 1;
        playerEntity->GetClient()->animation.endFrame = FRAME_salute11;
        break;
    case 2:
        gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_HIGH, "taunt\n");
        playerEntity->GetState().frame = FRAME_taunt01 - 1;
        playerEntity->GetClient()->animation.endFrame = FRAME_taunt17;
        break;
    case 3:
        gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_HIGH, "wave\n");
        playerEntity->GetState().frame = FRAME_wave01 - 1;
        playerEntity->GetClient()->animation.endFrame = FRAME_wave11;
        break;
    case 4:
    default:
        gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_HIGH, "point\n");
        playerEntity->GetState().frame = FRAME_point01 - 1;
        playerEntity->GetClient()->animation.endFrame = FRAME_point12;
        break;
    }
}
...
void Cmd_Say_f(PlayerEntity* playerEntity, qboolean team, qboolean arg0) {
    ...
    // Fetch client.
    ServerClient* cl = playerEntity->GetClient();
    ...
    if (team)
        message = "(" + std::string(cl->persistent.netname) + "): ";
    else
        message = "" + std::string(cl->persistent.netname) + ": ";
    ...
    if (flood_msgs->value) {
        cl = playerEntity->GetClient();

        if (!cl)
            return;

        if (level.time < cl->flood.lockTill) {
            gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_HIGH, "You can't talk for %d more seconds\n",
        ...
        if (cl->flood.when[i] &&
            ...
            gi.CPrintf(playerEntity->GetEntityServerHandle(), PRINT_CHAT, "Flood protection:  You can't talk for %d seconds.\n",
    ...
    for (j = 1; j <= game.maximumClients; j++) {
        PlayerEntity* other = dynamic_cast<PlayerEntity*>(serverGameEntities[j]);
        if (!other->IsInUse())
            continue;
        if (!other->GetClient())
            continue;
        ...
        gi.CPrintf(other->GetEntityServerHandle(), PRINT_CHAT, "%s", message.c_str());
    }
}
...
{
    PlayerEntity* playerEntity = dynamic_cast<PlayerEntity*>(serverGameEntities[clientEntity->state.number]);
    ...
    if (Q_stricmp(cmd, "players") == 0) {
        Cmd_Players_f(playerEntity);
        return;
    }
    if (Q_stricmp(cmd, "say") == 0) {
        Cmd_Say_f(playerEntity, false, false);
        return;
    }
    if (Q_stricmp(cmd, "say_team") == 0) {
        Cmd_Say_f(playerEntity, true, false);
        return;
    }
    ...
    if (Q_stricmp(cmd, "use") == 0)
        ...
        Cmd_Inven_f(clientEntity);
    else if (Q_stricmp(cmd, "invnext") == 0)
        SelectNextItem(playerEntity, -1);
    else if (Q_stricmp(cmd, "invprev") == 0)
        SelectPrevItem(clientEntity, -1);
    else if (Q_stricmp(cmd, "invnextw") == 0)
        SelectNextItem(playerEntity, ItemFlags::IsWeapon);
    else if (Q_stricmp(cmd, "invprevw") == 0)
        SelectPrevItem(clientEntity, ItemFlags::IsWeapon);
    else if (Q_stricmp(cmd, "invnextp") == 0)
        SelectNextItem(playerEntity, ItemFlags::IsPowerUp);
    else if (Q_stricmp(cmd, "invprevp") == 0)
        SelectPrevItem(clientEntity, ItemFlags::IsPowerUp);
    else if (Q_stricmp(cmd, "invuse") == 0)
        Cmd_InvUse_f(playerEntity);
    else if (Q_stricmp(cmd, "invdrop") == 0)
        Cmd_InvDrop_f(playerEntity);
    else if (Q_stricmp(cmd, "weapprev") == 0)
        Cmd_WeapPrev_f(playerEntity);
    else if (Q_stricmp(cmd, "weapnext") == 0)
        Cmd_WeapNext_f(playerEntity);
    else if (Q_stricmp(cmd, "weaplast") == 0)
        Cmd_WeapLast_f(playerEntity);
    else if (Q_stricmp(cmd, "kill") == 0)
        Cmd_Kill_f(playerEntity);
    else if (Q_stricmp(cmd, "putaway") == 0)
        Cmd_PutAway_f(clientEntity);
    else if (Q_stricmp(cmd, "wave") == 0)
        Cmd_Wave_f(playerEntity);
    else if (Q_stricmp(cmd, "playerlist") == 0)
        Cmd_PlayerList_f(clientEntity);
    else    // anything that doesn't match a command will be a chat
        Cmd_Say_f(playerEntity, false, true);
}
